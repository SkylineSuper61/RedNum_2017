...
/**
 * 这个实现了下拉刷新和上拉加载更多的功能，实现了IPullToRefresh接口，同时继承了LinearLayout,若要实现ListView、ScrollView的下拉刷新，
 * 只需要分别扩展这个类，实现一些必要的功能即可
 *
 * @param <T> <T extends View> 表示此泛型类的对象类型必须是继承 View 类
 * @author Li Hong
 * @since 2013-7-29
 */
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    public static boolean FIRST_ITEM_VISIBLE = false; //用于标识ListView中的第一项是否完全可见
    public static boolean LAST_ITEM_VISIBLE = false; //用于标识ListView的最后一项是否完全可见
     ...
     * 上拉加载是否可用
     */
    private boolean mPullLoadEnabled = /*false*/ true;
    /**
     * 判断滑动到底部加载是否可用
     */
    private boolean mScrollLoadEnabled = /*false*/ true;
    /**
     * 是否截断touch事件
     */
    ...
    public final boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                    ...
                    // 3，isPullLoading()，是否正在上拉加载更多
                    if (absDiff > mTouchSlop || isPullRefreshing() || isPullLoading()) {
                        mLastMotionY = event.getY();
                        // 第一个显示出来，Header已经显示或拉下
                        // isPullRefreshEnabled() : 判断当前下拉刷新是否可用
                        // isReadyForPullDown() : 判断刷新的View是否滑动到顶部  // ===> 此处的刷新View指的是装ListView的ScrollView
                        if (isPullRefreshEnabled() /*&& isReadyForPullDown() */&& FIRST_ITEM_VISIBLE) {
                            ...
                            //isPullLoadEnabled() : 判断上拉加载是否可用
                            //isReadyForPullUp() : 判断刷新的View是否滑动到底
                        } else if (isPullLoadEnabled() && /*isReadyForPullUp()*/ LAST_ITEM_VISIBLE) {
                            // 原理如上
                            Log.i("yang0", "====> 已经到底部");
                            mIsHandledTouchEvent = (Math.abs(getScrollYValue()) > 0 || deltaY < -0.5f);
                            if (mIsHandledTouchEvent) {
                                Log.i("yang0", "=========> bottom action");
                                mRefreshableView.onTouchEvent(event);
                            }
                        }
                    }
    ...
    public final boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                Log.i("yang0", "=========> ACTION_UP");
                if (mIsHandledTouchEvent) {
                    Log.i("yang0", "=========> mIsHandledTouchEvent");
                    mIsHandledTouchEvent = false;
                    // 当第一个显示出来时
                    if (/*isReadyForPullDown()*/FIRST_ITEM_VISIBLE) {
                        // 调用刷新
                        if (mPullRefreshEnabled && (mPullDownState == ILoadingLayout.State.RELEASE_TO_REFRESH)) {  // =========================>上拉到顶部刷新
                            startRefreshing();
                            Log.i("yang0", "=========> startRefreshing调用下拉刷新");
                            handled = true;
                        }
                        resetHeaderLayout();
                    } else if (/*isReadyForPullUp()*/LAST_ITEM_VISIBLE) {  //==========================> 下拉到底部加载更多
                        // 加载更多
                        Log.i("yang0", "====> Loading More Item上拉加载更多");
     ...
     * <p>
     * 本类的 createHeaderLoadingLayout()/createFooterLoadingLayout()方法将中间的content和顶部header/底部footer联系在一起
     */
